...
cumulative use across the session. That is confounded with, not a substitute for, the per-request
...
credited elsewhere works against success here. That is a tool-design tension, not a reasoning failure.
...
Under the current architecture the narration is an evidence channel feeding a decomposition, not a
category of its own.
...
with no budget tension it scores slightly better. Bigger is not a substitute for measuring the task.
