...
Groq solved it every time, citing the correct two other transactions and the correct arithmetic
(e.g. "order_87eaaadad2f0 (+20000) and order_1eb48d7ec70d (+15000) together offset the -35000 delta").
The local model mostly didn't — one run emitted a garbled, non-native tool-call attempt inside its
own content field instead of a real tool call; others gave up ("no other transaction... explains
this") despite the tool result containing the answer; one genuinely got it right. Raw evidence for
both providers, all 8 seeds each: `docs/evidence/multiway-netting-experiment-2026-08-28.json`
(generated by `scripts/generate_multiway_netting_evidence.py`).

**Two honest findings, kept as found, not smoothed into one convenient story**: the rule really can't
do this, provably; a capable model reliably can, through genuine compositional reasoning over raw
data rather than forwarding an oracle's answer; and a smaller local model mostly can't either — "an
LLM helps" isn't a blanket claim, it depends on which one. README's "Where the LLM actually sits"
paragraph and a new "One task the rule provably can't do" section both rewritten to carry this
honestly, replacing language that implied all three shipped categories required genuine LLM judgment.
...
pass/fail unit test). 202/202 total suite passing.
