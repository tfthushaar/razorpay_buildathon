...
not live pointers to a file that still exists.
...
A closer re-read of the forecaster's own README claim -- "backtested honestly against real ground
truth -- 9.1% MAPE, 93.3% interval coverage" -- found it reads as genuine forecast uncertainty to a
judge, when a large share of that accuracy is mechanical: `predict_settlement()` calls the exact same
`fee_and_tax()` function the generator itself used to compute the "real" fee/tax on every
non-anomalous transaction, so the prediction is bound to match the actual to the paise wherever no
refund, dispute, or timing anomaly was injected. That's the one place in this build's own headline
copy where the framing had run ahead of the mechanism.

**Verified independently before rewriting anything, not just asserted**:
...
**Judged, not just patched**: reusing the merchant's own known, contracted fee schedule and SLA
...
fix needed was disclosure, not a methodology change. (A stronger future version of this test --
perturbing the SLA constants inside the generator without telling the predictor, to build a
genuinely blind test -- is a real, separate idea worth considering for a future round, not
implemented here: it would change what's being measured, not just how it's described, and deserves
its own deliberate pass rather than being folded into a same-day correction.)
...
The same re-read also turned up that category discovery (Phase 3) produced six distinct proposed
names across eight `genuine_error` cases, five of them singletons -- a genuine pattern would recur
across cases, and independently-prompted, un-clustered proposals don't. Confirmed this directly
against `docs/evidence/discovery-ollama-run-2026-08-27.json`. Judged this one as not requiring a fix:
the feature is already named and described honestly as an "unreviewed hypothesis" a human reviews,
never as a taxonomy that self-organizes into recurring categories. Left as a documented, real
limitation rather than a problem to paper over -- worth a real clustering pass (matching new
proposals against prior ones in the same run) in a future round, not invented today just to look
more finished.
...
## 2026-08-28 — The mock narrator scores 100%, and one task the shipped rule provably can't do

A closer look at what the narrator's classification task actually requires turned up something
that changes how the whole "calibrated autonomy" story should be told.

**Measured directly, not assumed**: ran `narrate_mock` (a 20-line if/elif over `check_batch_anomalies`'s
own output, zero LLM calls) against real ground truth across 7 seeds, main and stress batches, 519
real narration-queue cases:
...
Perfect, on every category the real LLM narrator is reserved for. The real Ollama/Groq narrator does
not match this — 98.3% on `netting_trap`, 80.3% on `genuine_error` (README's own "The result"
figures). **On this specific classification task, the deterministic rule is a strict upgrade over the
LLM, not the reverse.**

The cause is structural, confirmed by reading both sides of it: `_gen_duplicate_refund`
(`app/data_gen/generate.py`) injects a shortfall that equals exactly one refund on record;
`_gen_netting_trap` injects an exact ±X pair inside one settlement batch; `_gen_genuine_error`
injects a junk delta matching nothing. `check_batch_anomalies` (`app/narrator/tools.py`) checks
exactly those same three conditions. The same author wrote the injector and the detector, so there
is no genuine ambiguity left for a classifier to resolve — `narrate_mock` isn't solving anything, it's
reading an oracle's answer back correctly, and the real LLM narrator's job on these three categories
is to reproduce that same oracle read reliably under real conditions (API failures, malformed tool
args, a hallucinated id) it structurally can't be wrong about *unless* something goes wrong along the
way. That's a genuine "Failure Recovery" property, not an "AI Judgment on ambiguous cases" one, and
the two had been conflated in how this project told the story.

**Rather than leave that as the only finding, built and measured one task the rule genuinely can't
do.** `check_batch_anomalies`'s netting check is pairwise-only: does one other transaction in the
same batch have the exact opposite delta. A group of three-or-more transactions whose deltas cancel
*together*, with no pair among them cancelling alone, is structurally invisible to it — not a missed
case, a limit of what the function checks by design. `app/narrator/multiway_netting_experiment.py`
hand-constructs exactly that: three transactions in one settlement batch, deltas +20000, +15000,
-35000 (paise) — cancel as a group, no pair does. Confirmed directly (`test_multiway_netting_experiment.py`,
deterministic, not sampled): `check_batch_anomalies` finds nothing on any of the three, every time.

A new tool, `list_batch_deltas`, exposes exactly the raw data `check_batch_anomalies` already has
internally (every other transaction's delta in the same batch) without doing the summing itself —
deliberately not a combinatorial (subset-sum) rewrite of the rule, which is a real, available fix,
disclosed as an option rather than quietly taken. Across 8 seeds, asked a real model to explain the
target transaction's delta with only that tool available:

```
groq (openai/gpt-oss-20b):        8/8 correct
ollama (qwen2.5:7b-instruct):     1/8 correct
```

Groq solved it every time in this run, citing the correct two other transactions and the correct
arithmetic (e.g. "order_87eaaadad2f0 (+20000) and order_1eb48d7ec70d (+15000) together offset the
-35000 delta"). The local model mostly didn't in this run: one run emitted a garbled, non-native
tool-call attempt inside its own content field instead of a real tool call; one genuinely got it
right; the rest failed to produce a parseable final answer within the tool-call budget. Raw evidence
for both providers, all 8 seeds each: `docs/evidence/multiway-netting-experiment-2026-08-28.json`
(generated by `scripts/generate_multiway_netting_evidence.py`).

**A closer re-read of this exact evidence file the next day found this experiment's design itself had
real problems — see the 2026-08-28 (II) entry below for the full account and the corrected, harder
re-run's real numbers.** Kept here rather than silently rewritten, since a visible correction is more
credible than one that pretends the first version never happened.

6 new tests (`test_multiway_netting_experiment.py`, covering only the deterministic half — the
LLM half is inherently non-deterministic and reported as measured evidence, not asserted as a
pass/fail unit test). 202/202 total suite passing at this point in the build (superseded below).

---

## 2026-08-28 (II) — Multi-way netting experiment: five real methodology holes, found and fixed

A closer re-read of the previous entry's own evidence file, the next day, turned up five real
problems with the experiment's design — not disagreements about framing, checkable facts:

1. **Correlated samples, not independent ones.** `deltas = [20000, 15000, -35000]` was hardcoded in
   `build_experiment_case` — only ids and timestamps varied across the 8 "seeds," so all 8 trials
   were the identical arithmetic puzzle relabeled, not independent evidence. This is exactly the
   pattern `MIN_DISTINCT_TRANSACTIONS_FOR_AUTO_RESOLVE` (`app/calibration/calibrator.py`) exists to
   reject elsewhere in this project.
2. **No distractors.** The batch held exactly 3 transactions, so "the other 2" was the only
   non-trivial candidate group — not a search, a forced move.
3. **The prompt named the strategy.** The system prompt said outright "look for a GROUP... whose
   deltas... sum to (approximately) zero" — handing over the solution method, not just the tool.
4. **The grader passed on over-citation.** `set(group_ids).issubset(set(cited))` — with only 2 real
   "other" transactions in the batch, citing everything seen trivially satisfied it regardless of
   whether any real arithmetic happened.
5. **The Ollama "1/8" narrative didn't match its own linked evidence.** BUILD_LOG's previous entry
   said the model's failures were it "giving up... despite the tool result containing the answer."
   Re-reading `docs/evidence/multiway-netting-experiment-2026-08-28.json` directly: 6 of 8 raw
   responses were `JSONDecodeError: Expecting value` (empty content — the harness exhausting its
   3-round tool-call budget without the model ever producing a final answer, not the model
   reasoning and giving up), 1 was a hallucinated transaction id echoed back as an "explanation,"
   and 1 was genuinely correct. Zero of the 8 showed the model actually attempting the group
   arithmetic and reaching a wrong conclusion — the claimed narrative simply wasn't in the file it
   cited. On a project whose whole differentiator is that claims survive checking, this is the most
   expensive kind of error available, and it's fixed by rewriting the false sentence in place in the
   entry above rather than left standing next to a correction.

**All five fixed, not patched around:**

- `build_experiment_case` now derives the target delta and the group's two-way split from
  `random.Random(seed * 7919 + 1)` — a genuinely different arithmetic puzzle per seed, verified by a
  new test (`test_different_seeds_produce_genuinely_different_arithmetic`) that 5 seeds produce 5
  distinct signatures, not 1.
- The batch now includes `N_DISTRACTORS = 8` unrelated transactions alongside the real 2-member
  group (11 "other" transactions total, not 2) — drawn from a wide, fine-grained delta range
  specifically because a first attempt at this used only 8 fixed round-number values and produced a
  real accidental subset-sum collision (caught by the uniqueness check below, not shipped
  unnoticed). `build_experiment_case` now brute-forces every non-empty subset up to size 4 of the
  "other" transactions and raises `AssertionError` if any subset besides the real group also cancels
  the target's delta — verified across seeds 1-29 in `test_construction_raises_if_ever_ambiguous_
  rather_than_silently_shipping_a_bad_case`.
- The system prompt now says only what the tool returns, not what pattern to look for.
- Grading is now exact-match (`set(cited) == set(group_ids)`) — citing every id seen now fails, since
  9 of the 11 "other" transactions are wrong answers.
- The harness now distinguishes "exhausted the tool-call round budget without ever producing a final
  answer" (a real, distinct failure mode, reported as such) from "produced a final answer that
  didn't parse" — matching the production narrator's own posture (`app/narrator/agent.py`'s
  `_fail_safe("...did not converge within the tool-call budget...")`) instead of feeding empty
  content to `json.loads` and reporting a confusing parse error.
- The "519 real narration-queue cases, 100.0%" figure from the previous entry was itself run as an
  ad-hoc one-liner, never committed — the same unreproducible-headline-number failure mode this
  project has caught and fixed in itself before. Now `scripts/measure_mock_narrator_accuracy.py`,
  committed, reproduces it exactly: `519/519 = 100.0%` (`duplicate_refund` 96/96, `genuine_error`
  117/117, `netting_trap` 306/306).

**The corrected, harder experiment's real result — genuinely different from the first attempt, and
weaker for both providers, which is exactly what removing the answer-leak and adding real distractors
should do:**

```
ollama (qwen2.5:7b-instruct, local):  0/8 correct
groq (openai/gpt-oss-20b):            4/8 correct
```

(One Groq attempt hit a real rate-limit error (429, tokens-per-minute) on the first pass, not a
reasoning failure — retried once after a cooldown for a clean read, disclosed as a `note` field
directly in the evidence file rather than silently re-run and merged in.)

This time the failures are genuinely qualitative, not harness artifacts, and neither model's
failures are flattering: Ollama twice flatly claimed no other transactions existed in the batch when
the tool result held 10 of them; both models, when wrong, tended to cite one or two *distractor*
transactions with large, plausible-*sounding* deltas and hand-wave a "these explain it" conclusion
without the cited numbers actually summing to anything — confident prose over arithmetic that
doesn't check out, on both providers. Groq's 4 correct answers all showed the real, verifiable sum
in the explanation text itself, which is what makes them trustworthy rather than lucky citations.

**Kept as found, harder result and all** — a 4/8 and a 0/8 make a less dramatic README line than the
first attempt's 8/8 and 1/8, and that is exactly why the first attempt's numbers shouldn't have been
trusted: they were easier to get than they should have been. README's "One task the rule provably
can't do" section and BUILD_LOG's own accounting rewritten to match. 3 new tests
(`test_the_search_space_has_real_distractors_not_just_the_correct_pair`,
`test_different_seeds_produce_genuinely_different_arithmetic`,
`test_construction_raises_if_ever_ambiguous_rather_than_silently_shipping_a_bad_case`), 207/207 total
suite passing.
...
work — reading the code as someone trying to break the claims, not confirm them. The score attached
to each round was an internal tracking device against a rubric written for that purpose, not a claim
about quality on its own. The *shape* of it is the evidence, not the number itself: the score dropped
to 83 in round 4 (down from round 3's 84) and to 72 in round 5, both times because a pass found
something real. A smooth, monotonic climb across 27 rounds would have meant the loop had stopped
finding anything — this one didn't.
