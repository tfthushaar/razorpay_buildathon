...
- [x] **Narrator tool-call execution guard** — self-caught (not an audit finding) while writing up the previous fix: the tool-call branch had the identical unguarded-crash problem one call earlier (malformed tool-call arguments, a hallucinated/unknown tool name). Same fix pattern: wrapped in try/except, routes through `_fail_safe`, 2 new tests written first and confirmed to fail against the unfixed code — see BUILD_LOG. Every model-supplied value in the narrator loop is now guarded. 63/63 tests passing.
...
- [x] FastAPI app: POST /api/run, GET /api/runs/latest, GET /api/calibration (live dial), POST /api/escalations/resolve (feedback loop), GET /api/audit, POST /api/transactions/evaluate (live "break it" scenario eval), GET /api/health — 61/61 tests passing overall (current total as of the most recent audit round — see BUILD_LOG.md; per-file counts elsewhere in this file describe that section's own coverage at the time it was completed, not a running total)
