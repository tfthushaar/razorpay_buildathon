...
def _timing_comparison() -> list[dict]:
    print("--- timing: brute force vs. optimal, real measurements ---")
    entries = []
    for n in TIMING_SIZES:
        case = build_scale_case(seed=1, n_total=n, group_size=3)
        ...
        print(f"  n_total={n}: optimal={optimal_secs:.5f}s ({optimal.algorithm_used}), brute_force={bf_str}")
    return entries
...
def _ambiguity_sweep() -> list[dict]:
    ...
    for n in AMBIGUITY_SIZES:
        ...
        entries.append({"n_total": n, "true_group_found": correct, "seeds": AMBIGUITY_SEEDS, "true_match_rate": rate})
        print(f"  n_total={n}: true group found {correct}/{AMBIGUITY_SEEDS} = {rate:.0%}")
    return entries
